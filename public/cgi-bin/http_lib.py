...
def generate_token(data: dict) -> str:
    ...
    header: bytes = base64.b64encode(
        json.dumps({"alg": "HS256", "typ": "JWT"}).encode("ascii")
    )
    payload: bytes = base64.b64encode(json.dumps(data).encode("ascii"))

    secret = SECRETS["JWT_SEED"].encode("utf-8")
    signature: str = hmac.new(
        secret, header + ".".encode("ascii") + payload, hashlib.sha256
    ).hexdigest()
...
def verify_token(jwt: str) -> Tuple[bool, str, dict]:
    ...
    try:
        ...
        logger.debug(
            f"Failed to decode header from bytes {base64.b64decode(header).decode('ascii')}"
        )
    ...
    if header_d.get("typ") != "JWT" or header_d.get("alg") != "HS256":
        logger.debug(f"Header validation failed, received {header_d}")
        return False, "Invalid header", {}

    test_sig = hmac.new(
        secret, (header + "." + payload).encode("ascii"), hashlib.sha256
    ).hexdigest()
    ...
    if test_sig == signature:
        try:
            ...
            logger.debug(
                f"Failed to decode payload from bytes {base64.b64decode(payload).decode('ascii')}"
            )
            return False, "Failed to decode payload", {}
        except Exception:
            logger.debug(
                f"Failed to decode payload {payload}: {traceback.format_exc()}"
            )
            return False, "Unknown error likely with base64 decoding", {}
...
def params(method: str = "GET") -> dict:
    ...
    if "REQUEST_METHOD" in environ:
        if environ["REQUEST_METHOD"] != method:
            raise HttpResponse(
                405,
                f"Expected {method} request but received {environ['REQUEST_METHOD']} request",
            )
    ...
    if method == "POST":
        ...
        if not content_type.startswith("multipart/"):
            ...
            def on_file(file):
                ...
                if (
                    isinstance(file_obj, BytesIO)
                    and file_obj.getbuffer().nbytes == 0
                    or isinstance(file_obj, BufferedRandom)
                    and file_obj.tell() == 0
                ):
                    logger.info(
                        "No file uploaded. Skipping and continuing with decoding."
                    )
                    return
                ...
                path = os.path.join(DOWNLOAD_FOLDER, new_filename)
                ...
                try:
                    ...
                    scale = 500 / width
                    image.resize((500, int(height * scale)), Image.Resampling.LANCZOS)
                    image.save(path)
                except UnidentifiedImageError:
                    logger.warning("Image decoding error, skipping decoding")
                    logger.debug(
                        "Image decoding traceback:\n%s", traceback.format_exc()
                    )
                    return

                param_data[key] = {"filename": new_filename, "path": path}
